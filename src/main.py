...
from utils.notifications import send_low_balance_alert # Модуль для мониторинга баланса и отправки через телеграм бота
...
class ChatApp:
    ...
    def update_balance(self):
        ...
        try:
            ...
            numeric_balance = float(balance.replace('$', '')) # Переводим из строки во float

            if numeric_balance < 10.0: # Если меньше, чем указано, отправляем алёрт в телеге
                send_low_balance_alert('@anxiety_squid', balance)
